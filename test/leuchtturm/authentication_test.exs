defmodule Leuchtturm.AuthenticationTest do
  use Leuchtturm.DataCase

  alias Leuchtturm.Authentication

  import Leuchtturm.AuthenticationFixtures
  alias Leuchtturm.Authentication.{User, UserToken}
  ...
  describe "register_user/1" do
    ...
    test "validates email and password when given" do
      {:error, changeset} =
        Authentication.register_user(%{email: "not valid", password: "not valid"})
  ...
  describe "update_user_email/2" do
    ...
    test "does not update email if user email changed", %{user: user, token: token} do
      assert Authentication.update_user_email(%{user | email: "current@example.com"}, token) ==
               :error
  ...
  describe "reset_user_password/2" do
    ...
    test "updates the password", %{user: user} do
      {:ok, updated_user} =
        Authentication.reset_user_password(user, %{password: "new valid password"})
